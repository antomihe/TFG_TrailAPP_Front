// app\(logged)\dashboard\(federation)\events\jury\components\EventJuryList.tsx
...
import {
    AlertTriangle, ServerCrash, ListX, Loader2, ClipboardList, UsersRound as PageIcon
} from 'lucide-react';
import { PaginationComponent } from '@/components/ui/pagination-component';
import { Small } from '@/components/ui/typography';

import { useFetchJuryEvents, JuryEvent } from '@/hooks/api/dashboard/federation/useFetchJuryEvents';
...
export default function EventsJuryList() {
    const { events, loading, error, refetch: refetchEvents } = useFetchJuryEvents();
    ...
    const columns: ColumnDef<JuryEvent>[] = useMemo(() => getJuryEventTableColumns(), []);
            ...
            {(!events || events.length === 0) && !loading && !error ? (
                <div className="px-1 sm:px-0">
                    <CenteredMessage
                        ...
                        title="Sin Eventos Validados"
                        message="No hay eventos validados por la federación para asignar jurado en este momento."
                    />
                </div>
                ...
                <div className="border rounded-lg overflow-hidden dark:border-neutral-800 shadow-md bg-card">
                    <ScrollArea className="max-h-[70vh] w-full">
                        <Table className="min-w-[360px] sm:min-w-full"> {/* Adjusted min-w for very small screens */}
