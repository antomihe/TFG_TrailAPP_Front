// app\(logged)\dashboard\(federation)\events\manage\components\EventManagerList.tsx
...
import {
    AlertTriangle, ServerCrash, ListX, Loader2, ClipboardList, Settings2 as PageIcon
} from 'lucide-react';
import { PaginationComponent } from '@/components/ui/pagination-component';
import { Small } from '@/components/ui/typography';

import { useFetchManagerEventsData, ManagerEvent } from '@/hooks/api/dashboard/federation/useFetchManagerEventsData';
import { getManagerEventsTableColumns } from './EventsTableColumns';
import { ManagerEventsTableControls } from './EventsTableControls';
...
export default function EventsManagerList() {
    const {
        ...
        loading,
        error,
        sending: isProcessingAction,
        handleDeleteEvent,
        refetchEvents,
    } = useFetchManagerEventsData();
    ...
    const columns: ColumnDef<ManagerEvent>[] = useMemo(() => getManagerEventsTableColumns({
        onDeleteEvent: handleDeleteEvent,
        isDeleting: isProcessingAction,
    }), [handleDeleteEvent, isProcessingAction]);
    ...
    const getVisibleColumnCountForSkeleton = () => {
        ...
        columns.forEach(col => {
            ...
            if (col.enableHiding === false) {
                count++;
            } else if (!columnVisibility[col.id as string] === false) {
                count++;
            } else if (
                !meta?.headerClassName?.includes('hidden sm:table-cell') &&
                !meta?.headerClassName?.includes('hidden md:table-cell') &&
                !meta?.headerClassName?.includes('hidden lg:table-cell')
            ) {
                count++;
            }
        });
    ...
    return (
        <div className="container mx-auto px-1 sm:px-2 py-6 space-y-6">
            <ManagerEventsTableControls table={table} />
            ...
            {(!events || events.length === 0) && !loading && !error ? (
                <div className="px-1 sm:px-0">
                    <CenteredMessage
                        ...
                        title="Sin Eventos"
                        message="No hay eventos creados o asociados a tu federación en este momento."
                    />
                </div>
                ...
                <div className="border rounded-lg overflow-hidden dark:border-neutral-800 shadow-md bg-card">
                    <ScrollArea className="max-h-[70vh] w-full">
                        <Table className="min-w-[420px] sm:min-w-full"> {/* min-w for very small screens with ~3 columns */}
