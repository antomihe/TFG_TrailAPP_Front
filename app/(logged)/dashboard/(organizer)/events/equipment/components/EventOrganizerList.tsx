// app\(logged)\dashboard\(organizer)\events\equipment\components\EventOrganizerList.tsx
...
import { useFetchOrganizerEquipmentEventsData, OrganizerEquipmentEvent } from '@/hooks/api/dashboard/organizer/useFetchOrganizerEquipmentEventsData';
import { OrganizerEquipmentEventsTableControls } from './OrganizerEquipmentEventsTableControls';
import { getOrganizerEquipmentEventsTableColumns } from './OrganizerEquipmentEventsTableColumns';
...
export default function EventsOrganizerList() { 
    ...
    } = useFetchOrganizerEquipmentEventsData();
    ...
    const columns: ColumnDef<OrganizerEquipmentEvent>[] = useMemo(() => getOrganizerEquipmentEventsTableColumns(), []);
        ...
        return (
          <div className="container mx-auto px-2 sm:px-4 py-8">
            <CenteredMessage
              ...
              variant="destructive"
              message={<>Ocurrió un problema al obtener los datos de eventos para la gestión de material. <br/> ({fetchError || "Error desconocido"})</>}
              action={refetchData && (
    ...
    if (events.length === 0 && !fetchError) {
        return (
          <div className="container mx-auto px-2 sm:px-4 py-8">
             <OrganizerEquipmentEventsTableControls
                table={table}
            ...
            <CenteredMessage
              ...
              variant="default"
              message="No tienes eventos futuros asignados como organizador para la gestión de material, o ninguno coincide con los filtros."
            />
          </div>
        );
    }
